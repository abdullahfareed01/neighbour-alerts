...
// ─── Shared token maps ────────────────────────────────────────────────────────
const TAG = {
  Theft:        "bg-amber-50   dark:bg-amber-900/30  text-amber-700  dark:text-amber-300  border-amber-300  dark:border-amber-700",
  Robbery:      "bg-red-50     dark:bg-red-900/30    text-red-700    dark:text-red-300    border-red-300    dark:border-red-700",
  Assault:      "bg-orange-50  dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border-orange-300 dark:border-orange-700",
  Burglary:     "bg-purple-50  dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border-purple-300 dark:border-purple-700",
  Vandalism:    "bg-blue-50    dark:bg-blue-900/30   text-blue-700   dark:text-blue-300   border-blue-300   dark:border-blue-700",
  "Lost Item":  "bg-sky-50     dark:bg-sky-900/30    text-sky-700    dark:text-sky-300    border-sky-300    dark:border-sky-700",
  "Item Found": "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700",
  Snatching:    "bg-pink-50    dark:bg-pink-900/30   text-pink-700   dark:text-pink-300   border-pink-300   dark:border-pink-700",
  Harassment:   "bg-rose-50    dark:bg-rose-900/30   text-rose-700   dark:text-rose-300   border-rose-300   dark:border-rose-700",
};
const EMOJI = {
  Theft:"💰", Robbery:"🔫", Assault:"🚨", Burglary:"🏠",
  Vandalism:"🔨", "Lost Item":"📦", "Item Found":"✅",
  Snatching:"🏃", Harassment:"⚠️",
};
...
export default function UserProfile() {
  ...
  useEffect(() => {
    setLoading(true);
    incidentsAPI.getUserIncidents(CURRENT_USER.id)
      .then(({ data }) => {
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1 mt-1 text-xs text-gray-400 dark:text-slate-500">
                  <Calendar size={11} />
                  <span>Joined {new Date(CURRENT_USER.joinedAt).toLocaleDateString("en-US", { month: "short", year: "numeric" })}</span>
                </div>
              </div>
            </div>
