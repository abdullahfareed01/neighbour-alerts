 ...
 *
 * FIX: colors/emoji and the high/moderate/safe danger-level bucketing now
 * come from constants/incidentTypes.js instead of being duplicated locally
 * (the duplicated dangerTypes/moderateTypes arrays here could previously
 * drift out of sync with utils/dangerScore.js's own classification).
 */

import React, { useMemo, useEffect, useState } from "react";
import { X, TrendingUp, AlertTriangle, Shield, Clock } from "lucide-react";
import { TYPE_HEX_COLOR as TYPE_COLORS, TYPE_EMOJI, TYPE_DANGER_TIER } from "../../constants/incidentTypes";
...
export default function Analyticspanel({
  ...
  const stats = useMemo(() => {
    ...
    // Danger level analysis — bucketed via the shared TYPE_DANGER_TIER map
    // (same categorization utils/dangerScore.js uses per-incident) instead
    // of a second hardcoded high/moderate/safe type list.
    const dangerLevels = { high: 0, moderate: 0, safe: 0 };
    incidents.forEach((inc) => {
      const tier = TYPE_DANGER_TIER[inc.type] ?? "safe";
      dangerLevels[tier]++;
    });
